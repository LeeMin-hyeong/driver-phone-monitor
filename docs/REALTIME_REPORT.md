...
| 지표 | 기존 확정 모델 | 실시간 모델 |
|---|---:|---:|
| Accuracy | 88.36% | 93.84% |
| Precision | 85.71% | 90.48% |
| Recall | 93.51% | 98.70% |
| F1 | 89.44% | 94.41% |

- 혼동행렬: TN 61, FP 8, FN 1, TP 76.
- 평균 **22.63ms / 44.19 FPS**, p95 **29.49ms**, 최대 70.54ms.
- 19회 × 146장 = 2,774회 처리, 누적 측정 구간 62.8초. 모든 반복에서 네 지표 기준을 충족했다.
- 루프 부가 비용까지 포함한 실제 처리율 44.06 FPS, 실제 시각에 기반한 이동 1초 구간의 최소 처리량 27장/초.
- 환경: NVIDIA GeForce RTX 4050 Laptop GPU, batch 1, 모델 상주, CPU 스레드 1. 측정된 장비/조건의 결과이며 모든 프레임의 하드 실시간 마감 보장은 아니다.
...
```mermaid
flowchart LR
    A[640 JPEG 입력] --> B[운전자 영역 YOLO26m]
    A --> C[전체 화면 YOLO26n-pose]
    B --> D[객체·손목 관계 특징]
    C --> D
    D --> E[벡터화 ExtraTrees]
    E -->|점수 ≥ 0.25| P[phone]
...
“객체 검출·자세 추정·이진 분류 파이프라인을 분석해 중복 검출을 제거하고, GPU 자세 모델과 손목 이미지 보완 분류를 결합했습니다. 학습된 트리의 판정을 유지하는 벡터화 실행과 TensorRT를 적용해 640 입력에서 44.2 FPS를 측정했으며, 동일한 146장 평가셋에서 F1 94.41%, Recall 98.70%로 기존 지표를 유지했습니다.”
